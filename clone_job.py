...
class SmartClone(openQAHelper):
    ...
    def run(self, jobid, dryrun: bool = False):
        try:
            ids = jobid.split(',')
            for one_id in ids:
                self.shell_exec(f'{self.cmd} {one_id} {self.params_str}', log=True, dryrun=dryrun)
        except Exception:
            self.handle_error()
    ...
    def query(self, query: str, dryrun: bool = False):
        ...
        if query.startswith('allpc'):
            ...
            for job in unique_jobs:
                self.shell_exec(f'{self.cmd} {job.id} {self.params_str}', log=True, dryrun=dryrun)
            ...
            for job in unique_jobs:
                ...
                self.shell_exec(f'{self.cmd} {job.id} {new_params}', log=True, dryrun=dryrun)
        else:
